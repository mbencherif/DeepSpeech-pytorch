...
def main(args):
    ...
    hmp_keys =["level","verbose","bf16ops", "fp32ops"]
    hmp_params= dist().fromkeys(hmp_keys)
    hmp_params["level"]="01"
    hmp_params["verbose"]=False
    hmp_params[""]="./ops_fp16_mnist.txt"
    hmp_params[""]="./ops_fp32_mnist.txt"
    ...
    # 3 INIT TRAINER
    # ------------------------
    trainer = Trainer(hpus=1, max_epochs=1,precision=16, hmp_params=hmp_params,callback=[lr_logger,early_stop,checpoint_callback], logger=logger)
    #trainer = Trainer(
        #gradient_clip_val=0,
